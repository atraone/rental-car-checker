 ...
 * Standalone script that calls the backend API directly
 */

import { writeFileSync } from 'fs';
import { join } from 'path';

const API_BASE_URL = process.env.API_BASE_URL || 'https://atra.one';
const KIE_API_KEY = process.env.KIE_API_KEY || 'b4e6228e96413e7f5e2f7323b93b8180';

async function generateIcon() {
  // Create a minimal base image (1x1 transparent PNG)
...
- Glass: Clean, reflective, blank/empty`;
  ...
  try {
    ...
    const apiUrl = `${API_BASE_URL}/api/kie`;
